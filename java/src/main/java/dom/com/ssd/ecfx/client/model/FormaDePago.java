...
public class FormaDePago {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!FormaDePago.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<FormaDePago> ta = gson.getDelegateAdapter(this, TypeToken.get(FormaDePago.class));
      return (TypeAdapter<T>) new TypeAdapter<FormaDePago>() {
        @Override public void write(JsonWriter out, FormaDePago v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public FormaDePago read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static FormaDePago fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, FormaDePago.class); }
