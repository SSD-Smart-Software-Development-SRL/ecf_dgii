...
public class FormaDePagoMontoPago {
  public FormaDePagoMontoPago() {}
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!FormaDePagoMontoPago.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<FormaDePagoMontoPago> ta = gson.getDelegateAdapter(this, TypeToken.get(FormaDePagoMontoPago.class));
      return (TypeAdapter<T>) new TypeAdapter<FormaDePagoMontoPago>() {
        @Override public void write(JsonWriter out, FormaDePagoMontoPago v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public FormaDePagoMontoPago read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static FormaDePagoMontoPago fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, FormaDePagoMontoPago.class); }
