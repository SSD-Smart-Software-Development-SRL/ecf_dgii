...
public class ItemPrecioUnitarioItem {
  public ItemPrecioUnitarioItem() {}
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!ItemPrecioUnitarioItem.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<ItemPrecioUnitarioItem> ta = gson.getDelegateAdapter(this, TypeToken.get(ItemPrecioUnitarioItem.class));
      return (TypeAdapter<T>) new TypeAdapter<ItemPrecioUnitarioItem>() {
        @Override public void write(JsonWriter out, ItemPrecioUnitarioItem v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public ItemPrecioUnitarioItem read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static ItemPrecioUnitarioItem fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, ItemPrecioUnitarioItem.class); }
