...
public class OtraMonedaTipoCambio {
  public OtraMonedaTipoCambio() {}
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!OtraMonedaTipoCambio.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<OtraMonedaTipoCambio> ta = gson.getDelegateAdapter(this, TypeToken.get(OtraMonedaTipoCambio.class));
      return (TypeAdapter<T>) new TypeAdapter<OtraMonedaTipoCambio>() {
        @Override public void write(JsonWriter out, OtraMonedaTipoCambio v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public OtraMonedaTipoCambio read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static OtraMonedaTipoCambio fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, OtraMonedaTipoCambio.class); }
