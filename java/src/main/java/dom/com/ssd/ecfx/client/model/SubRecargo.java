...
public class SubRecargo {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!SubRecargo.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<SubRecargo> ta = gson.getDelegateAdapter(this, TypeToken.get(SubRecargo.class));
      return (TypeAdapter<T>) new TypeAdapter<SubRecargo>() {
        @Override public void write(JsonWriter out, SubRecargo v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public SubRecargo read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static SubRecargo fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, SubRecargo.class); }
