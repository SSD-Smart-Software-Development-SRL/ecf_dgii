...
public class SubcantidadItem {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!SubcantidadItem.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<SubcantidadItem> ta = gson.getDelegateAdapter(this, TypeToken.get(SubcantidadItem.class));
      return (TypeAdapter<T>) new TypeAdapter<SubcantidadItem>() {
        @Override public void write(JsonWriter out, SubcantidadItem v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public SubcantidadItem read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static SubcantidadItem fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, SubcantidadItem.class); }
