...
public class TotalesMontoPeriodo {
  public TotalesMontoPeriodo() {}
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    @SuppressWarnings("unchecked") @Override public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      if (!TotalesMontoPeriodo.class.isAssignableFrom(type.getRawType())) return null;
      final TypeAdapter<JsonElement> ea = gson.getAdapter(JsonElement.class);
      final TypeAdapter<TotalesMontoPeriodo> ta = gson.getDelegateAdapter(this, TypeToken.get(TotalesMontoPeriodo.class));
      return (TypeAdapter<T>) new TypeAdapter<TotalesMontoPeriodo>() {
        @Override public void write(JsonWriter out, TotalesMontoPeriodo v) throws IOException { ea.write(out, ta.toJsonTree(v).getAsJsonObject()); }
        @Override public TotalesMontoPeriodo read(JsonReader in) throws IOException { return ta.fromJsonTree(ea.read(in)); }
      }.nullSafe();
    }
  }
  public static TotalesMontoPeriodo fromJson(String s) throws IOException { return JSON.getGson().fromJson(s, TotalesMontoPeriodo.class); }
